...
export function useDockState() {
  ...
  const startDrag = useCallback(
    (position: Position) => dispatch({ type: 'START_DRAG', position }),
    [],
  )
  const commit = useCallback(
    (position: Position, mode: DockMode) => dispatch({ type: 'COMMIT', position, mode }),
    [],
  )
  const returnToDock = useCallback(() => dispatch({ type: 'RETURN_TO_DOCK' }), [])
  const restore = useCallback((state: DockState) => dispatch({ type: 'RESTORE', state }), [])
